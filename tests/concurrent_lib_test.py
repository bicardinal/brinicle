...
class VectorEngineConcurrentTests:
	...
	def test_concurrent_reads(self):
		...
		db.close()
		print(f"[OK] {n_threads} threads performed {n_threads * searches_per_thread} searches successfully")
	...
	def test_concurrent_writes_multiprocess(self):
		...
		for i in range(n_processes):
			p = Thread(target=insert_worker, args=(i, inserts_per_process, result_queue))
			processes.append(p)
			p.start()
		...
		print(f"[OK] {n_processes} processes inserted {n_processes * inserts_per_process} vectors")
	...
	def test_concurrent_deletes(self):
		...
		def delete_worker(proc_id, ids_to_delete, result_queue):
			try:
				...
				local_db.close()
				result_queue.put(("success", proc_id, deleted_count, len(not_found) if not_found else 0))
			except Exception as e:
				result_queue.put(("error", proc_id, str(e)))
		...
		print(f"[OK] {n_processes} processes attempted deletions, total deleted: {total_deleted}")
	...
	def test_concurrent_upserts(self):
		...
		print(f"[OK] {n_processes} processes performed overlapping upserts successfully")
		...
		def operation_worker(worker_id, op_type):
			try:
				...
				if op_type == "search":
					...
					ids = [f"vec_{i}" for i in range(worker_id * 10, worker_id * 10 + 10)]
