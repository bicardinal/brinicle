...
class VectorEngineIrrationalTests:
	...
	def run_all(self):
		self.setup()
		try:
			test_methods = [getattr(self, m) for m in dir(self) 
	...
	def test_delete_nonexistent_ids(self):
		...
		deleted, not_found = db.delete_items(["nonexistent1", "nonexistent2"], 
	...
	def test_negative_delta_ratio(self):
		try:
			db = self.VectorEngine(self._get_test_path("neg_delta"), 
							  dim=128, delta_ratio=-0.5)
	...
	def test_very_large_delta_ratio(self):
		try:
			db = self.VectorEngine(self._get_test_path("large_delta"), 
							  dim=128, delta_ratio=10.0)
			assert False, "Should reject invalid delta_ratio"
		except:
			assert True
