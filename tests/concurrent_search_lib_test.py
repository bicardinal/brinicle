...
class VectorEngineConcurrentSearchTests:
        ...
        def search_worker(thread_id, n_searches, k):
            try:
                ...
                for i in range(n_searches):
                    ...
                    assert (
                        len(res) > 0
                    ), f"Thread {thread_id}: Empty results at iteration {i}"
                    assert (
                        len(res) <= k
                    ), f"Thread {thread_id}: Too many results ({len(res)} > {k})"
                    count += 1
    ...
    def test_search_during_insert(self):
        ...
        for i in range(n_inserters):
            t = Thread(
                target=batch_inserter, args=(i, batches_per_inserter, vectors_per_batch)
            )
            t.start()
    ...
    def test_search_during_delete(self):
        ...
        def continuous_searcher(thread_id):
            try:
                ...
                local_db.close()
                avg_results = (
                    sum(result_sizes) / len(result_sizes) if result_sizes else 0
                )
        ...
        def batch_deleter(deleter_id, n_rounds, delete_per_round):
            try:
                ...
                for round_num in range(n_rounds):
                    ...
                    ids_to_delete = [
                        f"vec_{i}" for i in range(start_id, start_id + delete_per_round)
                    ]
        ...
        for i in range(n_deleters):
            t = Thread(
                target=batch_deleter, args=(i, rounds_per_deleter, delete_per_round)
            )
            t.start()
    ...
    def test_search_during_upsert(self):
        ...
        for i in range(n_upserters):
            t = Thread(
                target=batch_upserter, args=(i, rounds_per_upserter, upsert_per_round)
            )
            t.start()
    ...
    def test_search_during_build(self):
        ...
        print(
            f"[OK] {total_searches} searches during full rebuild of {n_new_vectors} vectors"
        )
    ...
    def test_search_during_optimize(self):
        ...
        print(
            f"  Optimization triggered: {optimize_complete[0] if optimize_complete else 'N/A'}"
        )
        ...
        def modifier_worker():
            try:
                ...
                for round_num in range(4):
                    ...
                    ids = [
                        f"base_{i}" for i in range(round_num * 10, round_num * 10 + 5)
                    ]
                    local_db.delete_items(ids)
